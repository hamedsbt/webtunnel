...
RUN go build \
    -ldflags="-s -w" \
     -o "build/server" \
     gitlab.torproject.org/tpo/anti-censorship/pluggable-transports/webtunnel/main/server
...
COPY release/container/start-tor.sh /usr/local/bin
RUN chmod 0755 /usr/local/bin/start-tor.sh

# !!! This is a sudo setuid binary, CHANGE IT WITH CAUTION.
COPY release/container/chown-states.sh /usr/local/bin
...
COPY release/container/get-bridge-line.sh /usr/local/bin
